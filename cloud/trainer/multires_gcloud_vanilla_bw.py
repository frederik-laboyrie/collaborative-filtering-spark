...
def calculate_error(model, test_full, test_med, test_low, test_labels, mean_, std_,
    ...
    print('\n' * 10)
    file_content = """false long more leaky bigger max min VANILLA double layer not pool not batch norm + xavier everywhere + 
...
def multires_CNN(filters, kernel_size, top_neurons, dropout, full, med, low):
    ...
    fullres_branch = Conv2D(filters, (kernel_size, kernel_size),
                            activation=LeakyReLU(alpha=0.1), kernel_initializer='glorot_normal')(input_fullres)
    ...
    #fullres_branch = BatchNormalization()(fullres_branch)
    fullres_branch = Conv2D(filters, (kernel_size, kernel_size),
                            activation=LeakyReLU(alpha=0.1), kernel_initializer='glorot_normal')(fullres_branch)
    #fullres_branch = Conv2D(filters, (kernel_size, kernel_size),
    ...
    medres_branch = Conv2D(filters, (kernel_size, kernel_size),
                           activation=LeakyReLU(alpha=0.1), kernel_initializer='glorot_normal')(input_medres)
    ...
    #medres_branch = BatchNormalization()(medres_branch)
    medres_branch = Conv2D(filters, (kernel_size, kernel_size),
                           activation=LeakyReLU(alpha=0.1), kernel_initializer='glorot_normal')(medres_branch)
    #medres_branch = Conv2D(filters, (kernel_size, kernel_size),
    ...
    lowres_branch = Conv2D(filters, (kernel_size, kernel_size),
                           activation=LeakyReLU(alpha=0.1), kernel_initializer='glorot_normal')(input_lowres)
    ...
    #lowres_branch = BatchNormalization()(lowres_branch)
    lowres_branch = Conv2D(filters, (kernel_size, kernel_size),
                           activation=LeakyReLU(alpha=0.1), kernel_initializer='glorot_normal')(lowres_branch)
    #lowres_branch = Conv2D(filters, (kernel_size, kernel_size),
...
def train_model(train_files='hand-data', job_dir='./tmp/test1', kernel_size=5,
    ...
    full = np.reshape(full, [len(full), 128, 128, 3, 1])
    med = np.load(imagesio64)
    med = np.reshape(med, [len(med), 64, 64, 3, 1])
    low = np.load(imagesio32)
    low = np.reshape(low, [len(low), 32, 32, 3, 1])
